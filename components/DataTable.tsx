...
import { SEX_LABEL, AGE_SHORT, unitSuffix, visibleReliability } from "@/lib/format";
  ...
  return <div><ClientReady />
    ...
    <div className="table-scroll" role="region" aria-label="Tabla de resultados, desplazamiento horizontal" tabIndex={0}><table className="w-full tnum"><caption className="sr-only">Estimaciones con intervalo de confianza al 95%, coeficiente de variación y confiabilidad.</caption><thead><tr>{showIndicator&&th("ind")}{th("ambito")}{th("sex")}{th("age")}{th("estimate")}<th scope="col">IC 95%</th>{th("cv")}<th scope="col">Confiabilidad</th></tr></thead><tbody>
      {visible.map(r=><tr key={`${r.indicator_id}-${r.geo_level}-${r.region_id}-${r.sex}-${r.age_group}`} data-selected={r.geo_level==="region"&&selectedRegion===r.region_id}>
        ...
        <td>{SEX_LABEL[r.sex]}</td><td className="whitespace-nowrap">{AGE_SHORT[r.age_group]}</td><td className="text-right font-semibold whitespace-nowrap">{r.estimate.toFixed(1)}{unitSuffix(getIndicator(r.indicator_id)?.unit)}</td><td className="text-right whitespace-nowrap text-subink">{r.ci_low.toFixed(1)}–{r.ci_high.toFixed(1)}{unitSuffix(getIndicator(r.indicator_id)?.unit)}</td><td className="text-right">{r.cv==null?"—":`${r.cv.toFixed(1)}%`}</td><td><span className={`reliability ${r.reliab==="media"||r.reliab==="baja"?"caution":""}`}>{visibleReliability(r.cv,r.reliab)??"No disponible"}</span></td>
      </tr>)}
    </tbody></table>{!filtered.length&&<div className="empty-state"><p>No hay resultados con esta búsqueda.</p><button className="btn btn-secondary mt-3" onClick={reset}>Restablecer búsqueda y filtros</button></div>}</div>
    ...
    <p className="download-scope mt-3">El CSV incluye todas las filas de la búsqueda y los filtros actuales, en este orden; incluye también las páginas no visibles.</p>
  </div>;
}
